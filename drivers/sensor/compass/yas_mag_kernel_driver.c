...
#define GEOMAGNETIC_INPUT_NAME              "geomagnetic"//"magnetic_sensor"
//#define GEOMAGNETIC_INPUT_RAW_NAME          "magnetic_sensor"//"raw_magnetic_sensor"//"geomagnetic_raw"
...
#endif
#ifdef YAS_MAG_SUPPORT_ELLIPSOID
    int32_t transform_matrix[9];
#endif
...
static struct file_operations sensor_raw_fops = {
    .owner = THIS_MODULE,
...
#endif
#ifdef YAS_MAG_SUPPORT_ELLIPSOID
static DEVICE_ATTR(transform_matrix, S_IRUGO|S_IWUSR,
        geomagnetic_raw_transform_matrix_show, geomagnetic_raw_transform_matrix_store);
#endif
...
#endif
#ifdef YAS_MAG_SUPPORT_ELLIPSOID
    &dev_attr_transform_matrix.attr,
#endif
    NULL
};
...
/*read status TA/USB connect*/
static int mag_charging_status(int* ac_status, int* usb_status, int* chg_status)
{

	struct power_supply *psy_chg_status = power_supply_get_by_name("ab8500_chargalg");
	struct power_supply *psy_usb_status = power_supply_get_by_name("ab8500_usb");
	struct power_supply *psy_ac_status = power_supply_get_by_name("ab8500_ac");
	...
	value.intval = 0;
	psy_chg_status->get_property(psy_chg_status, POWER_SUPPLY_PROP_STATUS, &value);

		*chg_status = value.intval;


	value.intval = 0;
	psy_usb_status->get_property(psy_usb_status, POWER_SUPPLY_PROP_ONLINE, &value);

		*usb_status = value.intval;


	value.intval = 0;
	psy_ac_status->get_property(psy_ac_status, POWER_SUPPLY_PROP_ONLINE, &value);
...
void geomagnetic_manual_offset (void)
{
	int ret = 0;
	struct geomagnetic_data *data = i2c_get_clientdata(this_client);
	struct geomagnetic_data *mag_offset;
    struct yas_vector offset;
	static prev_status = 0;
	static int h = 0;
	int ac_status, usb_status, chg_status;

	offset = data->manual_offset;

	mutex_lock(&data->offset_mutex);

	ret = mag_charging_status(&ac_status, &usb_status, &chg_status);
	...
	mutex_unlock(&data->offset_mutex);
...
#ifdef YAS_SENSOR_KERNEL_DEVFILE_INTERFACE
            get_time_stamp(&tv);
            make_event_w_time(&ev[0], EV_ABS, ABS_X, magdata->xyz.v[0], &tv);
            make_event_w_time(&ev[1], EV_ABS, ABS_Y, magdata->xyz.v[1], &tv);
            make_event_w_time(&ev[2], EV_ABS, ABS_Z, magdata->xyz.v[2], &tv);
            make_event_w_time(&ev[3], EV_ABS, ABS_STATUS, accuracy, &tv);
...
#ifdef YAS_SENSOR_KERNEL_DEVFILE_INTERFACE
            get_time_stamp(&tv);
            make_event_w_time(&ev[0], EV_ABS, ABS_X, magdata->raw.v[0], &tv);
            make_event_w_time(&ev[1], EV_ABS, ABS_Y, magdata->raw.v[1], &tv);
            make_event_w_time(&ev[2], EV_ABS, ABS_Z, magdata->raw.v[2], &tv);
